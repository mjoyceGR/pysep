...
def update_shell(run_file,prems_model_dir,input_model,phys_file_name,cont_file_name, *arg, **kwargs):
	...
	print "inpute model (update_shell): ", input_model
	...
	### everything else
		...
		print >> run, "export LD_LIBRARY_PATH=$LD_LIBRARY_PATH:"+head+"/free_eos-2.2.1/lib"
		print >> run, "LD_LIBRARY_PATH=$LD_LIBRARY_PATH:$HOME/free_eos-2.2.1/lib"
...
def make_prems_models(mstar,amlt,zin,yin, *args, **kwargs):
	...
	#print "value of head (loc 1): ", head
...
def do_run(mstar,amlt,zin,yin, shell_file, cont_file,phys_file,\
	...
	update_cont(cont_file, amlt, zin, yin, NMOD,\
				endage=endage, which_comp=which_comp,
				asteroseismic=asteroseismic, head=head) 

	update_shell(shell_file,prems_model_dir,out_iso_name,\
				phys_file, cont_file,which_comp=which_comp,\
				output_dir=output_dir,asteroseismic=asteroseismic,\
				head=head)
	subprocess.call("pwd", shell=True)
	subprocess.call("bash "+shell_file, shell=True)
